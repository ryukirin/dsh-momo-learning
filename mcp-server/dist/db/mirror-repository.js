...
export class MirrorRepository {
     ...
     */
    saveToday(words, day, requestedLocalDate = day) {
        const syncId = randomUUID();
        const now = new Date().toISOString();
        const completeness = words.length >= 1000 ? 'partial' : 'unknown';
        const save = this.db.transaction(() => {
            this.db
                .prepare(`INSERT INTO mirror_sync VALUES (?, 'today_snapshot', ?, ?, '墨墨当天项目', 1000, ?, NULL, ?, ?, ?, NULL, ?, ?)`)
                .run(syncId, `today:${day}`, requestedLocalDate, words.length, `上游 order 的前 ${words.length} 项`, completeness, words.length ? 'success' : 'empty', now, now);
            const word = this.db
        ...
        VALUES (?, ?, ?, ?, 'current', ?, ?) ON CONFLICT(word_id) DO UPDATE SET spelling=excluded.spelling,last_seen_at=excluded.last_seen_at,latest_sync_id=excluded.latest_sync_id`);
            ...
            for (const value of words) {
                ...
                item.run(syncId, value.wordId, value.order ?? 0, value.isNew == null ? null : Number(value.isNew), value.isFinished == null ? null : Number(value.isFinished), day, value.firstResponse ?? null);
            }
        });
        save();
    ...
    latestToday() {
        return this.db
            .prepare(`SELECT * FROM mirror_sync WHERE scope_type='today_snapshot' AND outcome IN ('success','empty') ORDER BY finished_at DESC, rowid DESC LIMIT 1`)
            .get();
    }
    /**
     ...
     */
    listToday(syncId, pageSize, offset, responses) {
        const filter = responses === undefined || responses.length === 0
            ? ''
            : ` AND i.first_response IN (${responses.map(() => '?').join(',')})`;
        return this.db
            .prepare(`SELECT w.word_id AS wordId,w.spelling,i.upstream_order AS "order",i.is_new AS isNew,i.is_finished AS isFinished,i.first_response AS firstResponse
      FROM today_item i JOIN mirror_word w ON w.word_id=i.word_id WHERE i.sync_id=?${filter} ORDER BY i.upstream_order,i.word_id LIMIT ? OFFSET ?`)
            .all(syncId, ...(responses ?? []), pageSize, offset);
    }
    /**
    ...
    listTodayPage(syncId, options) {
        ...
        const words = this.db
         ...
         ORDER BY i.upstream_order,i.word_id LIMIT ? OFFSET ?`)
            .all(syncId, ...parameters, options.pageSize, options.offset);
        const total = this.db
            .prepare(`SELECT COUNT(*) AS count FROM today_item i WHERE i.sync_id=?${where}`)
            .get(syncId, ...parameters).count;
        return { words, total };
    }
    /**
     ...
     */
    todayResponseCounts(syncId) {
        const rows = this.db
            .prepare(`SELECT COALESCE(first_response, 'unknown') AS response, COUNT(*) AS count
     ...
     */
    latestObservation() {
        const row = this.db.prepare('SELECT MAX(observed_at) AS observedAt FROM study_record').get();
     ...
     */
    latestAddDate() {
        const row = this.db.prepare('SELECT MAX(add_date) AS addedAt FROM study_record').get();
     ...
     */
    allWords() {
        return this.db.prepare('SELECT word_id AS wordId, spelling FROM mirror_word').all();
    }
    /**
    ...
    saveQuizMistakes(day, mistakes) {
        ...
        const word = this.db.prepare(`INSERT INTO mirror_word(word_id,spelling,first_seen_at,last_seen_at,plan_state,plan_state_updated_at,latest_sync_id)
       VALUES(?,?,?,?,'current',?,NULL) ON CONFLICT(word_id) DO UPDATE SET spelling=excluded.spelling,last_seen_at=excluded.last_seen_at`);
        const upsert = this.db.prepare(`INSERT INTO quiz_mistake(word_id,spelling,first_missed_day,last_missed_day,miss_count,last_prompt,last_type,cleared_day)
       VALUES(?,?,?,?,1,?,?,NULL)
       ON CONFLICT(word_id) DO UPDATE SET
        ...
        this.db.transaction(() => {
            for (const mistake of mistakes) {
                ...
                upsert.run(mistake.wordId, mistake.spelling, day, day, mistake.prompt ?? null, mistake.type ?? null);
                if (seen)
                    repeated += 1;
                else
                    added += 1;
    ...
    listQuizMistakes(options) {
        ...
        const words = this.db
            .prepare(`SELECT word_id AS wordId, spelling, first_missed_day AS firstMissedDay,
         ...
         LIMIT ? OFFSET ?`)
            .all(options.pageSize, options.offset);
        const total = this.db.prepare(`SELECT COUNT(*) AS count FROM quiz_mistake ${where}`).get().count;
        return { words, total };
    }
    /** Count the mistake book by state, for the day's report. */
    quizMistakeSummary() {
        const row = this.db
            .prepare(`SELECT SUM(cleared_day IS NULL) AS open, SUM(cleared_day IS NOT NULL) AS cleared FROM quiz_mistake`)
            .get();
    ...
    listQuizCandidates(options) {
      ...
      LEFT JOIN today_item today ON today.word_id = record.word_id AND today.sync_id = ?`;
        const words = this.db
            .prepare(`SELECT record.word_id AS wordId, word.spelling,
         ...
         LIMIT ?`)
            .all(todaySyncId, gapCutoff, options.limit);
        const total = this.db.prepare(`SELECT COUNT(*) AS count ${from} WHERE ${where}`).get(todaySyncId, gapCutoff).count;
        return { words, total };
    }
    /**
     ...
     */
    recordPlanCoverage(covered, knownTotal) {
        const syncId = randomUUID();
        const now = new Date().toISOString();
        this.db
            .prepare(`INSERT INTO mirror_sync VALUES (?, 'study_record_window', ?, NULL, '整个计划的学习记录', ?, ?, ?, '全量回填', 'complete', 'success', NULL, ?, ?)`)
    ...
    latestStudyWindow() {
        return this.db
            .prepare(`SELECT * FROM mirror_sync WHERE scope_type='study_record_window' AND outcome IN ('success','empty') ORDER BY finished_at DESC, rowid DESC LIMIT 1`)
            .get();
    }
    listPlanning(scope, includeRemoved, pageSize, offset) {
        const clause = scope === 'removed_history'
            ? "plan_state='removed_from_current_plan'"
            : includeRemoved
                ? "plan_state IN ('current','removed_from_current_plan')"
                : "plan_state='current'";
        const words = this.db
            .prepare(`SELECT word_id AS wordId,spelling,plan_state AS planState FROM mirror_word WHERE ${clause} ORDER BY spelling LIMIT ? OFFSET ?`)
            .all(pageSize, offset);
        const total = this.db.prepare(`SELECT COUNT(*) AS count FROM mirror_word WHERE ${clause}`).get().count;
        return { words, total };
    }
    saveStudyWindow(words, scopeKey, knownTotal, sourceLimit = config.upstreamCollectionMax) {
        const syncId = randomUUID();
        const now = new Date().toISOString();
        const completeness = words.length >= sourceLimit
            ? 'partial'
        ...
        this.db.transaction(() => {
            this.db
                .prepare(`INSERT INTO mirror_sync VALUES (?, 'study_record_window', ?, NULL, '北京时间的下次学习日期', ?, ?, ?, ?, ?, ?, NULL, ?, ?)`)
                .run(syncId, scopeKey, sourceLimit, words.length, knownTotal ?? null, scopeKey, completeness, words.length ? 'success' : 'empty', now, now);
            const word = this.db
                .prepare(`INSERT INTO mirror_word(word_id,spelling,first_seen_at,last_seen_at,plan_state,plan_state_updated_at,latest_sync_id)
        VALUES(?,?,?,?,'current',?,?) ON CONFLICT(word_id) DO UPDATE SET spelling=excluded.spelling,last_seen_at=excluded.last_seen_at,plan_state='current',plan_state_updated_at=excluded.plan_state_updated_at,latest_sync_id=excluded.latest_sync_id`);
            ...
            const record = this.db.prepare(`INSERT INTO study_record(word_id,add_date,first_study_date,last_study_date,next_study_date,study_count,tags,observed_at,latest_sync_id)
        VALUES(?,?,?,?,?,?,?,?,?)
        ON CONFLICT(word_id) DO UPDATE SET
          ...
          latest_sync_id=excluded.latest_sync_id`);
            for (const value of words) {
                ...
                record.run(value.wordId, value.addDate ?? null, value.firstStudyDate ?? null, value.lastStudyDate ?? null, value.nextStudyDate ?? null, value.studyCount ?? null, value.tags ? JSON.stringify(value.tags) : null, now, syncId);
            }
            // 只有同一学习记录范围此前出现、但本次完整成功同步中缺失的词，才能标为移出。
            // 当天快照和其他范围的词不会参与比较；不完整、未知或失败同步绝不能改变既有规划状态。
            if (completeness === 'complete') {
                this.db
                    .prepare(`UPDATE mirror_word
    ...
    queryRecords(filter) {
        ...
        if (filter.tags !== undefined && filter.tags.length > 0) {
            conditions.push(`EXISTS (SELECT 1 FROM json_each(record.tags) WHERE json_each.value IN (${filter.tags
                .map(() => '?')
                .join(',')}))`);
        ...
        if (!filter.includeRemoved)
            conditions.push("word.plan_state = 'current'");
        ...
        const words = this.db
         ...
         LIMIT ? OFFSET ?`)
            .all(...parameters, filter.pageSize, filter.offset);
        const total = this.db
...
//# sourceMappingURL=mirror-repository.js.map