...
import {
  countStudyRecords,
  getStudyRecords,
  getStudyRecordsByWordIds,
  getStudyRecordsPage
} from '../api/study-records.js';
...
/** What one plan-wide walk covered, and the evidence it stopped on. */
interface WalkOutcome {
  ...
  stoppedBecause: 'end-of-plan' | 'max-pages' | 'bound-not-advancing';
  observedAt: string;
  /** Whether upstream's own total matched what the walk covered. */
  complete: boolean;
}

/** What one study-record refresh did, and the evidence it decided on. */
interface RefreshOutcome {
  /** Newest record observation the mirror already held, before this refresh. */
  lastObservedAt: string | null;
  /** Beijing days between that observation and today; `null` when nothing was stored. */
  gapDays: number | null;
  /** How the refresh chose its target set. */
  mode: 'day' | 'catch-up';
  /** How many records upstream returned. */
  words: number;
  /** How many upstream calls it took. */
  calls: number;
  /** Present in catch-up mode: what the plan-wide walk covered. */
  walk?: WalkOutcome;
}
...
const walkStudyRecords = async (
  ...
  const wordsBefore = (
    db.prepare('SELECT COUNT(*) AS count FROM study_record').get() as { count: number }
  ).count;
  ...
  let pages = 0;
  let recordsRead = 0;
  let stopped: WalkOutcome['stoppedBecause'] = 'end-of-plan';
  for (;;) {
    const page = await getStudyRecordsPage(client, { start, limit: options.pageSize });
    pages += 1;
    recordsRead += page.records.length;
    for (const record of page.records) covered.add(record.voc_id);
    mirror.saveStudyWindow(
      ...
      BACKFILL_SCOPE_KEY,
      undefined,
      Math.max(page.records.length, 1)
    );
    if (page.records.length < options.pageSize) break;
  ...
  if (complete) mirror.recordPlanCoverage(covered.size, knownTotal as number);
  return {
    pages,
    recordsRead,
 ...
 */
const refreshStudyRecords = async (
  mirror: MirrorRepository,
  db: Database.Database,
  client: MaimemoClient,
  day: string,
  ...
  const mode: 'day' | 'catch-up' = gapDays !== null && gapDays <= 0 ? 'day' : 'catch-up';
  if (mode === 'catch-up') {
...
export const registerTools = (
  ...
  server.registerTool(
    'maimemo_auth_status',
    { description: '只读本机安全凭证状态。', inputSchema: strictObject({}) },
    async () => {
      try {
        const configured = Boolean(secrets.get());
        return result(
          envelope(
            { configured, usable: configured, credentialStoreAvailable: secrets.available() },
            'local_mirror'
          )
        );
      } catch (error) {
      ...
      try {
        ...
        return result(
          envelope(
            ...
            'local_mirror',
            (latest?.completeness as 'complete' | 'partial' | 'unknown') ?? 'unknown',
            latest ? [] : ['尚无本地学习镜像，请在开始当日待办前同步。'],
            latest
          )
        );
      } catch (error) {
  ...
  server.registerTool(
    ...
    async ({ requestedLocalDate }) => {
      try {
        ...
        if (words.length > config.previewMax) warnings.push('完整词表请使用本地分页工具读取。');
  ...
  server.registerTool(
    ...
    {
      description:
        '只读本地镜像单词，支持本地分页。当天快照范围可按墨墨记录的作答结果与是否新学筛选，用于确定当天要考的词的集合。',
      inputSchema: strictObject({
    ...
    async ({ scope, pageSize, cursor, snapshotId, includeRemoved, responses, isNew }) => {
      try {
        ...
        if (scope === 'today_snapshot') {
          ...
          if (!current?.id)
            return result(
              envelope({ words: [], nextCursor: undefined }, 'local_mirror', 'unknown', [
                '尚无当天快照。'
              ])
            );
            ...
            envelope(
              {
                ...
                nextCursor:
                  listing.words.length === pageSize ? String(offset + pageSize) : undefined
              },
              'local_mirror'
            )
          );
  ...
  server.registerTool(
    ...
    { description: '只读本地当日待办。', inputSchema: strictObject({ todoId: z.string() }) },
    async ({ todoId }) => {
      try {
  ...
  server.registerTool(
    ...
    async ({ todoId, action, expectedCurrentItemId }) => {
      try {
        return result(
          envelope(todos.update(todoId, action, expectedCurrentItemId), 'local_mirror')
        );
      } catch (error) {
        return fail(error);
      }
  ...
  server.registerTool(
    ...
    async ({ start, end, limit }) => {
      try {
        ...
        const saved = mirror.saveStudyWindow(
  ...
  server.registerTool(
    ...
    async ({ wordIds }) => {
      try {
        ...
        const rows = db
          .prepare(
            `SELECT word_id AS wordId,kind,content,translation,source,updated_at AS updatedAt FROM word_supplement WHERE word_id IN (${wordIds.map(() => '?').join(',')})`
          )
          .all(...wordIds);
  ...
  server.registerTool(
    ...
    {
      description:
        '从整个镜像里挑当天要考的词，按不熟程度排序：本地错题 > 墨墨标记顽固 > 今天复习没记住 > 学得少；已排除今天刚新学的词。只读本地。',
      inputSchema: strictObject({
    ...
    async ({ limit, minDaysSinceStudy }) => {
      try {
        ...
        return result(
          envelope(
            { day, words: listing.words, total: listing.total, minDaysSinceStudy },
            'local_mirror',
            coverage === undefined ? 'unknown' : 'complete',
            warnings,
            coverage
          )
        );
  ...
  server.registerTool(
    ...
    {
      description:
        '记录本地出题中答错的词，供明天错题优先与日报使用；只写错题，不留答题流水；该词后来答对时可一并标记消除。',
      inputSchema: strictObject({
    ...
    async ({ day, mistakes, cleared }) => {
      try {
        ...
        const clearedCount = cleared.filter((wordId) =>
          mirror.clearQuizMistake(wordId, quizDay)
        ).length;
        return result(
          envelope(
            {
  ...
  server.registerTool(
    ...
    {
      description:
        '在本地计划的词里找与给定单词易混的形近词，用于给新学的词出辨识易错题；只读本地，不访问上游。',
      inputSchema: strictObject({
      ...
      try {
        const target =
          ...
          (wordId === undefined
            ? undefined
            : (
                db.prepare('SELECT spelling FROM mirror_word WHERE word_id=?').get(wordId) as
                  | { spelling: string }
                  | undefined
              )?.spelling);
        if (target === undefined)
  ...
  server.registerTool(
    ...
    {
      description:
        '判断此刻该做什么：今天是否还有待学词、是否该先去 App 学完、以及本地镜像是否已跟上今天新增进规划的词。今天没有待学词时会顺带把计划读到最新。',
      inputSchema: strictObject({})
    },
    async () => {
      try {
        ...
        let decision: 'finish-in-app' | 'ready' | 'nothing-due' | 'unknown';
        if (total === null) decision = 'unknown';
        else if (total === 0) decision = 'nothing-due';
        else if (finished !== null && finished < total) decision = 'finish-in-app';
        else decision = 'ready';

        const warnings: string[] = [];
        let walk: WalkOutcome | null = null;
        let newlyAdded: { total: number; words: Record<string, unknown>[] } | null = null;
        ...
        if (decision === 'nothing-due' && !addDateCurrent) {
          ...
          warnings.push(
            `本地镜像中最新的加入规划日期是 ${lastAddDate ?? '（无）'}，今天新增的词尚未纳入；今天学完后同步即可跟上。`
          );
        }
  ...
  server.registerTool(
    ...
    async ({ confirm }) => {
      try {
        if (!confirm)
          throw new MaimemoError(
            'CONFIRMATION_REQUIRED',
            '请显式传入 confirm:true 以清除本地学习数据。'
          );
