 ...
 */
const hasUniqueUpstreamOrder = (db, table) => /UNIQUE\s*\([^)]*upstream_order[^)]*\)/i.test(storedTableSql(db, table));
/** Recreate one table without its version-1 `upstream_order` uniqueness rule, keeping every row. */
const rebuildWithoutUniqueUpstreamOrder = (db, table, columns) => {
    if (!hasUniqueUpstreamOrder(db, table))
        return;
 ...
 */
const collapseStudyRecordToPerWord = (db) => {
    if (!tableColumns(db, 'study_record').includes('sync_id'))
        return;
    db.pragma('foreign_keys = OFF');
    try {
        db.transaction(() => {
 ...
 */
const addTodayResponseColumn = (db) => {
    if (tableColumns(db, 'today_item').includes('first_response'))
        return;
 ...
 */
const beijingDayOfSync = (alias = '') => `date(${alias}finished_at, '+8 hours')`;
/**
 ...
 */
const normalizeStoredDatesToBeijingDays = (db) => {
    for (const column of studyDateColumns) {
        ...
        db.prepare(`UPDATE study_record SET ${column} = date(${column}, '+8 hours')
       WHERE ${column} LIKE '%T%' AND date(${column}, '+8 hours') IS NOT NULL`).run();
    }
    ...
    db.prepare(`UPDATE mirror_sync SET scope_key = 'today:' || ${beijingDayOfSync()}
     WHERE scope_type = 'today_snapshot'
       AND ${beijingDayOfSync()} IS NOT NULL
       AND scope_key <> 'today:' || ${beijingDayOfSync()}`).run();
    db.prepare(`UPDATE today_item SET source_date = (
...
export const migrate = (db) => {
    ...
    db.prepare('INSERT INTO schema_migration(version) VALUES(?) ON CONFLICT(version) DO NOTHING').run(schemaVersion);
};
