...
export class TodoRepository {
     ...
     */
    keepsExistingSession(sessionId, syncId) {
        const entries = this.db.prepare('SELECT COUNT(*) AS count FROM todo_entry WHERE session_id=?').get(sessionId).count;
        if (entries > 0)
            return true;
        const items = this.db.prepare('SELECT COUNT(*) AS count FROM today_item WHERE sync_id=?').get(syncId).count;
        return items === 0;
    }
    open(syncId, localDate) {
        const existing = this.db
            .prepare(`SELECT id FROM todo_session WHERE local_session_date=? AND status='active' ORDER BY updated_at DESC LIMIT 1`)
            .get(localDate);
        if (existing?.id && this.keepsExistingSession(existing.id, syncId))
        ...
        const items = this.db
            .prepare('SELECT word_id,upstream_order FROM today_item WHERE sync_id=? ORDER BY upstream_order,word_id')
            .all(syncId);
        const id = randomUUID();
        ...
        this.db.transaction(() => {
            if (existing?.id)
                this.db
                    ...
                    .run(now, existing.id);
            this.db
                .prepare(`INSERT INTO todo_session(id,source_sync_id,local_session_date,current_entry_id,paused_entry_id,status,created_at,updated_at) VALUES(?,?,?,?,?,'active',?,?)`)
                .run(id, syncId, localDate, entryIds[0] ?? null, null, now, now);
            const insert = this.db.prepare('INSERT INTO todo_entry VALUES(?,?,?,?,?,?)');
            items.forEach((item, index) => insert.run(entryIds[index], id, item.upstream_order, item.word_id, 'pending', now));
        })();
        return this.get(id);
    }
    get(id) {
        const session = this.db.prepare('SELECT * FROM todo_session WHERE id=?').get(id);
        if (!session)
            throw new MaimemoError('TODO_NOT_FOUND', '未找到当日待办。');
        const entries = this.db
            .prepare(`SELECT e.*,w.spelling FROM todo_entry e JOIN mirror_word w ON w.word_id=e.word_id WHERE e.session_id=? ORDER BY e.upstream_order,e.word_id`)
            .all(id);
        return { ...session, entries };
    }
    update(id, action, expected) {
        const session = this.db.prepare('SELECT * FROM todo_session WHERE id=?').get(id);
        if (!session)
            throw new MaimemoError('TODO_NOT_FOUND', '未找到当日待办。');
        ...
        if (action === 'pause_current') {
            ...
            this.db.transaction(() => {
                ...
                this.db
                    .prepare('UPDATE todo_session SET current_entry_id=NULL,paused_entry_id=?,updated_at=? WHERE id=?')
            ...
            this.db.transaction(() => {
                ...
                this.db
                    .prepare('UPDATE todo_session SET current_entry_id=paused_entry_id,paused_entry_id=NULL,updated_at=? WHERE id=?')
                    .run(now, id);
            })();
        }
            ...
            this.db.transaction(() => {
                ...
                const next = this.db
                    .prepare(`SELECT id FROM todo_entry WHERE session_id=? AND status='pending' AND (upstream_order,word_id) > (SELECT upstream_order,word_id FROM todo_entry WHERE id=?) ORDER BY upstream_order,word_id LIMIT 1`)
                    .get(id, session.current_entry_id);
                this.db
